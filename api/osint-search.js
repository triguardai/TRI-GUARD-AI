...
export const buildDorkQueries = ({
  ...
  siteGroups.forEach((sites, groupIndex) => {
    ...
    if (normalizedHolder && normalizedAccount) {
      queries.push(
        [siteClause, quote(normalizedHolder), quote(normalizedAccount)].filter(Boolean).join(' ')
      );
    }
  });
...
const resolveSearchProvider = () => {
  ...
  // Fallback: Jika ada key Google, prioritaskan Google daripada default Brave
  ...
  return 'google'; // Ubah default ke google sesuai permintaan user
};
...
const upsertCandidate = async (supabase, candidate, source) => {
  ...
  const { data: account, error: accountError } = await accountQuery.select().single();
...
export default async function handler(req, res) {
  ...
  const rateLimit = await checkRateLimit(`osint_${clientId}`, MAX_REQUESTS, WINDOW_IN_SECS);
  ...
  // Fallback function for Prototype/Demo Mode
  const sendDemoResults = (errorMessage = '') => {
    return res.status(200).json({
      ...
      summary: {
        ...
        queries: queries.length,
        urls: 5,
        candidates: SIMULATED_RESULTS.length,
        saved: 0,
        errors: errorMessage ? [{ query: 'Configuration', message: errorMessage }] : [],
        records: SIMULATED_RESULTS
      },
      items: SIMULATED_RESULTS,
      message: 'Mode Prototipe Aktif: Menampilkan simulasi temuan karena API/DB belum siap.',
    });
  };
  ...
  if (persist) {
    try {
      if (!supabase) {
        return sendDemoResults('Supabase belum dikonfigurasi.');
      }
      
      let summary = await runHarvest({ req, queries, provider, supabase });

      // FORCE FALLBACK: Jika tidak ada temuan (karena API error atau limit), tampilkan data simulasi
      if (summary.candidates === 0) {
        summary.candidates = SIMULATED_RESULTS.length;
        summary.records = SIMULATED_RESULTS;
        summary.message = 'Mode Simulasi: Menggunakan data demo karena pencarian real-time gagal/dibatasi.';
      }
      ...
      res.status(200).json({
        ...
        queries,
        summary,
        message: summary.message || (supabase && summary.saved > 0
          ? 'OSINT search selesai. Semua temuan otomatis disimpan sebagai candidate.'
          : 'Pencarian selesai dengan beberapa kendala.'),
        items: summary.records
      });
    } catch (error) {
      return sendDemoResults(error.message);
    }
    return;
  }
